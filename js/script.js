...
tl.from("#sceneThreeDirectionText", {
  x: 100,
  delay: 1,
  opacity: 0,
  duration: 1
});
gsap.to("#pictureFilter feGaussianBlur", 1, {
  attr: { stdDeviation: 0 },
  repeat: -1,
  yoyo: true
});
...
function selectBuildings() {
  ...
  document
    .querySelectorAll(
      "#house_1_, #house, #house_2_, #house_3_, #house_4_, #house_14_, #house_15_, #house_16_, #houseFront"
    )
    .forEach(element => {
...
function showSceneThree(data) {
  ...
  document.querySelector("#sceneThreeText").innerHTML =
    data.scene_three[2].text;
  document.querySelector("#sceneThreeDirectionText").innerHTML =
    data.scene_three[2].direction_text;
}
